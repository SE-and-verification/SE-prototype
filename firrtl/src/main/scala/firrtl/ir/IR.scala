...
case class SubField(expr: Expression, name: String, tpe: Type = UnknownType, flow: Flow = UnknownFlow)
  ...
  def foreachType(f:  Type => Unit):             Unit = f(tpe)
  def foreachWidth(f: Width => Unit):            Unit = ()
  def mapToTag():Expression = this.copy(expr = expr.mapToTag)
}
...
case class SubIndex(expr: Expression, value: Int, tpe: Type, flow: Flow = UnknownFlow)
  ...
  def foreachType(f:  Type => Unit):             Unit = f(tpe)
  def foreachWidth(f: Width => Unit):            Unit = ()
  def mapToTag():Expression = expr.mapToTag
}
...
case class Mux(cond: Expression, tval: Expression, fval: Expression, tpe: Type = UnknownType)
  ...
  def foreachType(f:  Type => Unit):  Unit = f(tpe)
  def foreachWidth(f: Width => Unit): Unit = ()
  def mapToTag()                          : Expression = DoPrim(And, Seq(tval.mapToTag, fval.mapToTag), Seq[BigInt](), tpe.mapToTag)
...
case class ValidIf(cond: Expression, value: Expression, tpe: Type) extends Expression with UseSerializer {
  ...
  def foreachType(f:  Type => Unit):  Unit = f(tpe)
  def foreachWidth(f: Width => Unit): Unit = ()
  def mapToTag()                            : Expression = this.copy(cond= UIntLiteral(1), value = value.mapToTag, tpe = tpe.mapToTag)
}
...
case class UIntLiteral(value: BigInt, width: Width) extends Literal with UseSerializer {
  ...
  def mapToTag():                                Expression = UIntLiteral(1, IntWidth(1))
}
object UIntLiteral {
...
case class DoPrim(op: PrimOp, args: Seq[Expression], consts: Seq[BigInt], tpe: Type)
  ...
  def mapToTag: Expression = {
    if(args.size == 1){
      ...
      this.copy(op = And, args = args.map(_.mapToTag), tpe = tpe.mapToTag)
    }
  }
...
case class DefWire(info: Info, name: String, tpe: Type)
  ...
  def mapToTag: Statement = this.copy(tpe = tpe.mapToTag)
}
case class DefRegister(
  ...
  def foreachString(f: String => Unit): Unit = f(name)
  def foreachInfo(f:   Info => Unit):   Unit = f(info)
  def mapToTag: Statement = this.copy(tpe = tpe.mapToTag, init = init.mapToTag)
}
...
case class DefMemory(
  ...
  def foreachString(f: String => Unit):           Unit = f(name)
  def foreachInfo(f:   Info => Unit):             Unit = f(info)
  def mapToTag                                  : Statement = this.copy(dataType = dataType.mapToTag)
...
case class DefNode(info: Info, name: String, value: Expression)
  ...
  def foreachString(f: String => Unit):           Unit = f(name)
  def foreachInfo(f:   Info => Unit):             Unit = f(info)
  def mapToTag                                  : Statement = this.copy(value = value.mapToTag)
...
case class Conditionally(
  ...
  def foreachString(f: String => Unit):     Unit = ()
  def foreachInfo(f:   Info => Unit):       Unit = f(info)
  def mapToTag                                  : Statement = this.copy(pred = pred.mapToTag, conseq = conseq.mapToTag, alt = alt.mapToTag)
...
case class Block(stmts: Seq[Statement]) extends Statement with UseSerializer {
  ...
  def foreachString(f: String => Unit):           Unit = ()
  def foreachInfo(f:   Info => Unit):             Unit = ()
  def mapToTag                                  : Statement = this.copy(stmts.map(_.mapToTag))
...
case class PartialConnect(info: Info, loc: Expression, expr: Expression)
  ...
  def foreachString(f: String => Unit): Unit = ()
  def foreachInfo(f:   Info => Unit):   Unit = f(info)
  def mapToTag                                  : Statement = this.copy(loc = loc.mapToTag, expr = expr.mapToTag)
}
...
case class IsInvalid(info: Info, expr: Expression) extends Statement with HasInfo with UseSerializer {
  ...
  def foreachString(f: String => Unit):           Unit = ()
  def foreachInfo(f:   Info => Unit):             Unit = f(info)  
  def mapToTag                                  : Statement = this.copy(expr = expr.mapToTag)
}
...
case class Attach(info: Info, exprs: Seq[Expression]) extends Statement with HasInfo with UseSerializer {
  ...
  def foreachString(f: String => Unit):           Unit = ()
  def foreachInfo(f:   Info => Unit):             Unit = f(info)
  def mapToTag                                  : Statement = this.copy(exprs = exprs.map(_.mapToTag))
}
...
@data class Stop(info: Info, ret: Int, clk: Expression, en: Expression, @since("FIRRTL 1.5") name: String = "")
  ...
  def mapToTag:             Statement = this.copy(en = en.mapToTag)
}
object Stop {
...
@data class Print(
  ...
  def mapToTag: Statement = this.copy(args = args.map(_.mapToTag), en = en.mapToTag)
}
object Print {
...
case class Field(name: String, flip: Orientation, tpe: Type) extends FirrtlNode with HasName with UseSerializer{
  def mapToTag: Field = this.copy(tpe = tpe.mapToTag)
}
...
case class UIntType(width: Width) extends GroundType with UseSerializer {
  ...
  def foreachWidth(f: Width => Unit):  Unit = f(width)
  def mapToTag                : Type = UIntType(IntWidth(1))
}
...
case class BundleType(fields: Seq[Field]) extends AggregateType with UseSerializer {
  ...
  def mapToTag                : Type = this.copy(fields = fields.map(_.mapToTag))
...
case class Port(
  ...
  def mapToTag                      : Port = Port(info, name, direction, tpe.mapToTag)
}
...
case class ExtModule(
  ...
  def foreachString(f: String => Unit):         Unit = f(name)
  def foreachInfo(f:   Info => Unit):           Unit = f(info)
  def mapToTag                               : DefModule = this.copy(ports = ports.map(_.mapToTag))
}
...
case class Circuit(info: Info, modules: Seq[DefModule], main: String)
  ...
  def foreachInfo(f:   Info => Unit):           Unit = f(info)
  def mapToTag                                 : Circuit = this.copy(modules = modules.map(_.mapToTag))
}
