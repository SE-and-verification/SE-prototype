...
class SE(val debug : Boolean, val canChangeKey: Boolean) extends Module{
  ...
  seoperation.io.inst         := inst_buffer_buf
	...
	val op1_plaintext_64		= Mux(is_enc_const,  op1_buffer_after_decrypt_stage(127,64), Mux(op1_type_buffer_after_decrypt_stage, op1_bit(127, 64), op1_buffer_after_decrypt_stage(127,64))) // [plain_A]
