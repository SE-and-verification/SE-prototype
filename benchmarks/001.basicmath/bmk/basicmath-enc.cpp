...
//ISSUE!! KEY IS NOT INITIALIZED from setParams() before this global happens, so enc with wrong key!
//These globals must be re-initialized in Main()
                ...
                enc_double *x)
{
      ...
      if ((R2_Q3 <= 0).GET_DECRYPTED_VALUE())
      {
            *solutions = 3;
      ...
      else
      {
            *solutions = 1;
            ...
            x[0] = x[0] + (Q/x[0]);
            x[0] = x[0] * ((R < 0.0).GET_DECRYPTED_VALUE() ? 1 : -1);
            x[0] = x[0] - (a1/3.0);
      }
}
...
int main(void)
{
  ...
  for(a1=1;a1.GET_DECRYPTED_VALUE()<10.0;a1++) {
    for(b1=10;b1.GET_DECRYPTED_VALUE()>0.0;b1-=1.0) {
      for(c1=5;c1.GET_DECRYPTED_VALUE()<15.0;c1+=0.5) {
	      for(d1=-1;d1.GET_DECRYPTED_VALUE()>-11.0;d1-=1.0) {
	        SolveCubic(a1, b1, c1, d1, &solutions, x);  
	        printf("Solutions:");
	        for(i=0;i<solutions.GET_DECRYPTED_VALUE();i++)
	          printf(" %f",x[i].GET_DECRYPTED_VALUE());
